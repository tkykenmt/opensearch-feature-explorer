# Multi Plugin

| Document | Description |
|----------|-------------|
| [multi-plugin-build-infrastructure-gradle-jdk](multi-plugin-build-infrastructure-gradle-jdk.md) | Build Infrastructure (Gradle/JDK) |
| [multi-plugin-ci-cd-testing-infrastructure](multi-plugin-ci-cd-testing-infrastructure.md) | CI/CD & Testing Infrastructure |
| [multi-plugin-dependency-bumps](multi-plugin-dependency-bumps.md) | Dependency Bumps |
| [multi-plugin-gradle-9-compatibility](multi-plugin-gradle-9-compatibility.md) | Gradle 9 Compatibility |
| [multi-plugin-header-redesign](multi-plugin-header-redesign.md) | Header Redesign |
| [multi-plugin-jdk-21-java-agent-migration](multi-plugin-jdk-21-java-agent-migration.md) | JDK 21 & Java Agent Migration |
| [multi-plugin-look-and-feel-ui-improvements](multi-plugin-look-and-feel-ui-improvements.md) | Look & Feel UI Improvements |
| [multi-plugin-maintainer-updates](multi-plugin-maintainer-updates.md) | Maintainer Updates |
| [multi-plugin-multi-data-source-support](multi-plugin-multi-data-source-support.md) | Multi-Data Source Support |
| [multi-plugin-react-18-migration](multi-plugin-react-18-migration.md) | React 18 Migration |
| [multi-plugin-search-autocomplete](multi-plugin-search-autocomplete.md) | Search Autocomplete |
| [multi-plugin-testing-library-updates](multi-plugin-testing-library-updates.md) | Testing Library Updates |
| [multi-plugin-version-bumps-release-notes](multi-plugin-version-bumps-release-notes.md) | Version Bumps & Release Notes |
| [multi-plugin-version-increment](multi-plugin-version-increment.md) | Version Increment |
