...
- **v3.6.0**: Enabled Explore plugin in analytics (all use case) workspaces with nav link registration for `DEFAULT_NAV_GROUPS.all`. Explore appears as "Explorer" in analytics workspaces. Fixed nav link alias mapping from `data-explorer` to `explore` for correct active state highlighting. Fixed parent nav links to always be collapsible in nav groups. Added in-context visualization editor (`visualization-editor` app) for creating and editing visualizations directly within dashboards, with context-aware dataset filtering (PPL/PromQL/AI modes), edit routing based on creation origin, and save/discard with `stateTransfer` for dashboard round-trip navigation. Bug fixes: stateTransfer not passed when creating visualizations from dashboard dropdown, scope history out of scope for in-context editor (refactored `QueryBuilder.init()` to handle URL state restoration internally), visualization editor empty state components replaced with dedicated components removing legacy Discover dependency, visualization editor UI fixes for dropdown and edit URL routing.
