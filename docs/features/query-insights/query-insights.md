...
- **v2.19.0**: **Dashboards**: Bootstrapped query-insights-dashboards repository with CI/CD infrastructure - added basic CI workflow, GitHub Actions for 2.x branch, backport workflow, ESLint configuration with linting fixes, Mend security scanning integration, and post-build artifact renaming; **Plugin**: Fixed 2.x GitHub checks and CI by adding eclipse dependency in build.gradle
