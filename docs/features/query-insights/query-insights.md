...
- **v3.5.0**: Added user attribution for top N queries with `username` and `user_roles` fields capturing who ran each query; added dedicated settings API endpoints (`GET/PUT /_insights/settings`) for secure Dashboard configuration without requiring broad cluster settings permissions; optimized query storage by storing source field as string instead of SearchSourceBuilder (up to 58% memory savings for average queries); added source string truncation to prevent excessive storage; delayed user info extraction until after Top N filtering for performance; added missing mapping fields with enum-based validation tests; removed index template dependency in favor of explicit index mappings; retained local indices on exporter type change with new `LocalIndexLifecycleManager`; added `integTestRemote` Gradle target and `integTest` script for multi-node Jenkins testing; improved Cypress test pipeline with better dashboards readiness checks; fixed installation documentation
